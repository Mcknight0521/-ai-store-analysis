...
/* v5.18.1: anomaly page is display-only; threshold input belongs to import completion. */
...
(function(){
 ...
 function open(){const m=ensure();if(m.classList.contains('open'))return;const t=getT();document.getElementById('it518C').value=t?(Number(t.clearance)*100).toFixed(2).replace(/\.00$/,''):'';document.getElementById('it518W').value=t?(Number(t.waste)*100).toFixed(2).replace(/\.00$/,''):'';m.classList.add('open')}
 function check(){if(!window.confirmed||!Array.isArray(window.rows)||!window.rows.length)return;const s=sig();if(!s||sessionStorage.getItem(SESSION)===s)return;const m=ensure();if(m.classList.contains('open'))return;open()}
 ...
 // Poll only for import completion. Never rewrite input values while the modal is already open, so mobile typing/composition is preserved.
...
p.write_text(s,encoding='utf-8');print('v5.18.1 threshold typing reset fixed')
